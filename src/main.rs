use std::convert::TryInto;
use std::env;
use std::process;
use std::fs;
use std::fs::File;
use std::io::prelude::*;
//
pub struct Config {
    pub mode: String,
    pub key: String,
    pub input_file_path: String,
    pub output_file_path: String,
}
//
impl Config {
    fn build(args: &[String]) -> Result<Config, &'static str> {
        if args.len() < 5 {
            info();
            return Err("not enough arguments");
        }
        let mode = args[1].clone();
        let key  = args[2].clone();
        let input_file_path = args[3].clone();
        let output_file_path = args[4].clone();
        Ok(Config { mode,key, input_file_path, output_file_path })
    }
}
//
fn open_key_file(config: &Config) -> String {
    let key = fs::read_to_string(&config.key)
        .expect("Should have been able to read the key file");
    println!("key length: {}",key.len());
    key
}
//
fn read_input_file(config: &Config) -> (Vec<u16>, usize) {
    let bytes = fs::read(&config.input_file_path).unwrap();
    let mut vector_contents:Vec<u16>= vec![];
    let mut size = 0;
    for (index,byte_pair) in bytes.chunks_exact(2).enumerate() {
        let tmp = u16::from_le_bytes([byte_pair[0], byte_pair[1]]);
        vector_contents.push(tmp);
        size = index;
    }
    println!("Input file {} with size {}",&config.input_file_path,size*2);
    (vector_contents,size)
}
//
fn write_output_file(config: &Config, contents: Vec<u16>) {
    let mut file = File::create(&config.output_file_path).unwrap();
    let mut size = 0;
    for (index,value) in contents.iter().enumerate() {
       file.write_all(&value.to_le_bytes()).unwrap();
       size = index;
    }
    println!("Output file {} with size {}",&config.output_file_path,size*2);
}
//
fn transform(key: String) -> [u16;256]{
  let mut result:[u16;256]= [0; 256];
  for (index, y) in key[..256].chars().enumerate() {
        let z = (y.to_string()).parse::<u16>().unwrap() & 1;
        result[index] = z;
    }
  result
}
//
fn conv_data(a: [u16;16]) ->u16 {
  let mut result:u16=0;
  for (j, item) in  a.iter().enumerate() {
    result |= item;
    if j!=15 {
      result <<= 0x0001;
    }
  }
  result
}
//
fn info(){
  println!("AES Encryption Algorithm Version 0.1.0\n");
  println!("aes <options> <KEY file> <input file> <outputfile>");
  println!("<options> e: encryption, d: decryption");
}
//
fn mix_columns(s: &[[u16; 256]]) -> [[u16; 4]; 4]{
    let mut sp:[u16;4]=[0;4];
    let mut result = [[0u16; 4]; 4];
    for c in 0..4 {
       sp[0]= (0x02 | s[0][c])^(0x03|s[1][c])^      s[2][c] ^      s[3][c];
       sp[1]=         s[0][c]^ (0x02|s[1][c])^(0x03|s[2][c])^      s[3][c];
       sp[2]=         s[0][c]^       s[1][c]^ (0x02|s[2][c])^(0x03|s[3][c]);
       sp[3]= (0x03 | s[0][c])^      s[1][c]^       s[2][c] ^(0x02|s[3][c]);

       for i in 0..4 {
         result[i][c]=sp[i];
       }
    }
    result
}
//
fn inv_mix_columns(s: &[[u16; 256]]) -> [[u16; 4]; 4]{
    let mut sp:[u16;8]=[0;4];
    let mut result = [[0u16; 4]; 4];
    for c in 0..4 {
       sp[0]= (0x0e | s[0][c])^(0x0b | s[1][c])^(0x0d|s[2][c])^(0x09|s[3][c]);
       sp[1]= (0x09 | s[0][c])^(0x0e | s[1][c])^(0x0b|s[2][c])^(0x0d|s[3][c]);
       sp[2]= (0x0d | s[0][c])^(0x09 | s[1][c])^(0x0e|s[2][c])^(0x0b|s[3][c]);
       sp[3]= (0x0b | s[0][c])^(0x0d | s[1][c])^(0x09|s[2][c])^(0x0e|s[3][c]);

       for i in 0..4 {
         result[i][c]=sp[i];
       }
    }
    result
}
//
fn add_round_key(state: &[[u8; 4]], w:[u8; 16], mut keycount:u8 ){
     for c in 0..4 {
       for r in 0..4 {
         state[r][c]=state[r][c]^w[keycount++];
       }
     }
}
//
fn inv_add_round_key(state: &[[u8; 4]], w:[u8; 16], mut keycount:u8 ){
     for c in (0..3).rev() { 
       for r in (0..3).rev() {
         state[r][c]=state[r][c]^w[--keycount];
       }
     }
}
//
fn sub_bytes(state: &[[u8; 4]]) -> [[u8; 4]; 4]{
let s:[usize;256]=[  
    
 ];
    let mut result = [[u8; 4]; 4];
    for row in =0..4 {
      for col in 0..4 {
       result[row][col]=s[state[row][col]];
      }
    }
   result
}
//
fn inv_sub_bytes(state: &[[u8; 4]]) -> [[u8; 4]; 4]{
let inv_s:[usize;256]=[
    
 ]; 
    let mut result = [[u8; 4]; 4];
    for row in =0..4 {
      for col in 0..4 {
        result[row][col]=inv_s[state[row][col]];
       }
    }
  result
}
//
fn shift_rows(state: &[[u8; 4]]) -> [[u8; 4]; 4]{
    int []t = new int[4];
    for r in 1..4 {
      for c in 0..4 {
        t[c]=state[r][(c+r)%4];
      }
      for c in 0..4 {
        result[r][c]=t[c];
      }
    }
    result
}
//
fn inv_shift_rows(state: &[[u8; 4]]) -> [[u8; 4]; 4]{
    int []t = new int[4];
    let mut result = [[u8; 4]; 4];
    for r in 1..4 {
      for c in 0..4 {
        t[(c+r)%Nb]=state[r][c];
      }
      for c in 0..4 {
        result[r][c]=t[c];
      }
    }
    result
}
//
fn key_expansions(key:[u8;256]){
  let mut tmp [[0u8; 60]; 4];
  let mut i:usize = 0; 
  loop { 
     if i>=15 { return tmp; }
     tmp[i][0] = key[4*i]; 
     tmp[i][1] = key[4*i+1]; 
     tmp[i][2] = key[4*i+2]; 
     tmp[i][3] = key[4*i+3]; 
     i++; 
    } 
 tmp
} 
//
fn aes_encrypt(mut input:Vec<u16>,z:[u8;256],size:usize) -> Vec<u16>{
  let mut g:usize = 0;
  let mut j:usize = 0;
  let mut w:usize = 4;
  loop {
    if w>=size { return input; }
    key_expansions(z);
    add_round_key(state,z);
    for _i in 0..14 {
      sub_bytes(state);
      shift_rows(state);
      mix_columns(state);
      add_round_key(state,z);
    }
    sub_bytes(state);
    shift_rows(state);
    add_round_key(state,z);
    w+=16;
  }
}
//
fn aes_decrypt(mut input:Vec<u16>,z:[u8;256],size:usize) -> Vec<u16>{
  let mut g:usize = 0;
  let mut j:usize = 0;
  let mut w:usize = 4;
  loop {
    if w>=size { return input; }
    key_expansions(z);
    add_round_key(state,z);
    for _i in (0..14).rev() {
      inv_shift_rows(state);
      inv_sub_bytes(state);
      add_round_key(state,z);
      inv_mix_columns(state);
    } 
    inv_sub_bytes(state);
    inv_shift_rows(state);
    inv_add_round_key(state,z);
    w+=16
  }
}
// transforms a key/block String into an array of 256 bytes
fn transform(key: String) -> [u8;256] {
  let mut result:[u8;256]=[0;256];
  for (index,y) in key[..256].chars().enumerate() {
        result[index] = (y.to_string()).parse::<u8>().unwrap() & 1;
    }
  result
}
//
fn main(){
  let args: Vec<String> = env::args().collect();
  let config = Config::build(&args).unwrap_or_else(|err| {
        println!("Problem parsing arguments: {err}");
        info();
        process::exit(1);
    });
  let key = open_key_file(&config);
  let (vector_contents,size) = read_input_file(&config);
  println!("size:{size}");
  match config.mode.as_str() {
        // Encrypt
        "e" => {
            println!("Encrypt!");
            let encryption_keys = transform(key);
            let z = sub_key_gen(encryption_keys);
            let output = aes_encrypt(vector_contents,z,size);
            write_output_file(&config, output);
        },
        // Decrypt
        "d" => {
            println!("Decrypt!");
            let decryption_keys = transform(key);
            let z = sub_key_gen(decryption_keys);
            let u = dec_key_gen(z);
            let output = aes_decrypt(vector_contents,u,size);
            write_output_file(&config, output);
        },
        _ => info(),
    }
}
